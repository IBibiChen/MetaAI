...
- `ollama`：使用容器内 Ollama 本地模型服务，Chat 和 Embedding 共用一个 Ollama 容器，适合完整容器化单机部署
- `host-ollama`：使用宿主机已安装的 Ollama，Docker 只编排 Java 应用和基础设施服务

三套栈都包含 `meta-ai-agent`、Redis Stack、Qdrant、RustFS、PaddleX OCR 和 live-asr
...
`host-ollama` 栈不管理 Ollama 模型目录，模型由宿主机 Ollama 自行管理
...
Ollama 容器栈使用 `http://ollama:11434`，Host Ollama 栈使用 `http://host.docker.internal:11434`
