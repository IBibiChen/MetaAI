 ...
 *
 * <p>
 * RAG 异步文档索引调度器，负责 job 状态流转，并执行已组装好的 upsert pipeline
 *
 * <p>
...
public class MetaEtlPipeline {
     ...
     */
    @Async
    public void upsert(DocumentIndexingJob job, DocumentIndexingContext context) {
        jobRepository.save(job.withStatus(DocumentIndexingStatus.RUNNING, 0, "RAG document indexing started"));
        try {
            MetaEtlUpsertPipeline pipeline = pipelineFactory.createUpsertPipeline(context);
