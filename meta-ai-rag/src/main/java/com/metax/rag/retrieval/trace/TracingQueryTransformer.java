 ...
 * 转换后的 query 会进入 VectorStoreDocumentRetriever 做向量召回
 * 用户原始问题仍会在后续 ContextualQueryAugmenter 阶段和召回上下文一起组成最终 prompt
 *
 * <p>
 * 记录 queryTransform 耗时可以帮助判断一次 RAG 慢在检索前 query 转换，还是慢在向量库或模型生成
 * transformedQuery 可以帮助判断召回失败是因为知识库没有内容，还是因为 query 改写偏离了用户原意
 *
 * @author IBibiChen
 * @version v1.0
 * @since 2026/6/1
...
public class TracingQueryTransformer implements QueryTransformer {
    ...
    public Query transform(@NonNull Query query) {
        ...
        // 阶段 2：把转换结果写入 trace，便于 details 接口展示检索前发生了什么
