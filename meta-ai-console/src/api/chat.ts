...
export async function sendPlainChatJson(options: ChatOptions, fileIds: string[] = []) {
  const response = await request.post<CommonResult<ChatMessageResponse>>(
    '/v1/chat',
    removeEmptyFields({
...
export async function uploadChatFiles(
  ...
  const response = await request.post<CommonResult<MetaChatFileItem[]>>(
    '/v1/chat/files',
    formData,
    {
      headers: {
 ...
 */
export function streamPlainChatJson(
  options: ChatOptions,
  fileIds: string[],
  handlers: ChatStreamHandlers,
) {
  return openJsonChatStream(
    apiUrl('/v1/chat'),
    {
      chatId: options.chatId,
      tenantId: options.tenantId,
...
export async function sendRagChatJson(options: ChatOptions, fileIds: string[] = []) {
  const response = await request.post<CommonResult<RetrievalChatResponse>>(
    '/v1/rag',
    removeEmptyFields({
      chatId: options.chatId,
      msg: options.msg,
 ...
 */
export function streamRagChatJson(
  options: ChatOptions,
  fileIds: string[],
  handlers: ChatStreamHandlers,
) {
...
export async function fetchChatHistoryByChatId(chatId: string, current = 1, size = 200) {
  const response = await request.get<CommonResult<PageResult<MetaChatHistory>>>(
    '/v1/chat/history/page',
    {
      params: {
        chatId,
        current,
...
export async function updateChatFlags(
  id: string,
  flags: Partial<Pick<MetaChat, 'pinned' | 'favorite' | 'archived'>>,
) {
